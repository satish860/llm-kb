...
function buildQueryAgents(sourceFiles: string[], save: boolean, wikiContent: string): string {
  ...
  const lines = [
    ...
    `After your answer, include a CITATIONS block with bounding box data.`,
    ``,
    ...
    `CITATIONS:`,
    `[1] file: "document.md", page: 3, quote: "exact text", bbox: {x: 142, y: 340, width: 234, height: 14}`,
    `[2] file: "other.md", page: 7, quote: "exact text", bbox: {x: 98, y: 186, width: 422, height: 14}`,
    ``,
    `### Rules:`,
    `- The quote MUST be EXACT text from the source`,
    `- Always read the .json file to find bounding boxes for your citations`,
